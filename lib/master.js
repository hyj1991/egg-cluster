...
const ProcessAgentWorker = require('./utils/start_mode/process/agent');
const ProcessAppWorker = require('./utils/start_mode/process/app');
...
class Master extends EventEmitter {
  ...
  constructor(options) {
    ...
    if (this.options.startMode === 'process') {
  ...
  forkAgentWorker() {
    const worker = this.agentWorker.fork();
    this.workerManager.setAgent(worker);
  }

  forkAppWorkers() {
    const status = this.appWorker.fork();
    status.on('worker_forked', worker => this.workerManager.setWorker(worker));
  }
  ...
  killAgentWorker(timeout) {
    return this.agentWorker.kill(timeout);
  }
  ...
  onAgentExit(data) {
    ...
    const err = new Error(util.format('[master] agent_worker#%s:%s died (code: %s, signal: %s)',
      agentWorker.id, agentWorker.workerId, data.code, data.signal));
  ...
  onAgentStart() {
    ...
    this.messenger.send({
      action: 'egg-pids',
      to: 'app',
      data: [ this.agentWorker.workerId ],
    });
    ...
    this.logger.info('[master] agent_worker#%s:%s started (%sms)',
      this.agentWorker.id, this.agentWorker.workerId, Date.now() - this.agentWorker.startTime);
  }
  ...
  onAppStart(data) {
    ...
    this.log('[master] app_worker#%s:%s started at %s, remain %s (%sms)',
      worker.id, data.workerPid, address.port, remain, Date.now() - this.appWorker.startTime);
