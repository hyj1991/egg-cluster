...
class Master extends EventEmitter {
  ...
  forkAgentWorker() {
    const status = this.agentWorker.fork();
    status.on('agent_forked', agent => this.workerManager.setAgent(agent));
  }
  ...
  onAppExit(data) {
    ...
    if (!worker.isDevReload) {
      ...
      const message = util.format(
        ...
        worker.id, worker.workerId, worker.process.exitCode, signal,
  ...
  onAppStart(data) {
    ...
    this.log('[master] app_worker#%s:%s started at %s, remain %s (%sms)',
      worker.id, data.workerId, address.port, remain, Date.now() - this.appWorker.startTime);
