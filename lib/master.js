...
class Master extends EventEmitter {
  ...
  onAgentExit(data) {
    ...
    const err = new Error(util.format('[master] agent_worker#%s:%s died (code: %s, signal: %s)',
      agentWorker.id, agentWorker.worker_id, data.code, data.signal));
    ...
    if (this.isStarted) {
      ...
      this.logger.error('[master] agent_worker#%s:%s start fail, exiting with code:1',
        agentWorker.id, agentWorker.worker_id);
      process.exit(1);
    }
  }
  ...
  onAgentStart() {
    ...
    this.messenger.send({
      action: 'egg-pids',
      to: 'app',
      data: [ this.agentWorker.worker_id ],
    });
    ...
    this.logger.info('[master] agent_worker#%s:%s started (%sms)',
      this.agentWorker.id, this.agentWorker.worker_id, Date.now() - this.agentStartTime);
  }
